...
void init(DequeType* dq)         // 덱의 초기화
{
	dq->head = dq->tail = NULL;  // 덱의 처음과 끝을 NULL로 세팅
}

DlistNode* create_node(DlistNode* llink, element item, DlistNode* rlink)    // 노드 생성
{
...
void add_rear(DequeType* dq, element item)   // rear에 아이템 추가 연산
{
...
void add_front(DequeType* dq, element item)   // front에 아이템 추가 연산
{
